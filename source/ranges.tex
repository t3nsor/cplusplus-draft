...
\exposid{non-propagating-cache}<T> behaves exactly like \tcode{optional<T>}
with the following differences:
\begin{itemize}
\item
\exposid{non-propagating-cache}<T> constrains its type parameter \tcode{T}
...
\end{codeblock}
\item
\exposid{non-propagating-cache}<T> has an additional member function template
specified as follows:
\begin{itemdecl}
template<class I>
  constexpr T& @\exposid{emplace-deref}@(const I& i);    // exposition only
\end{itemdecl}
...
namespace std::ranges {
  ...
  private:
    ...
    V @\exposid{base_}@ = V();                      // \expos
    @\exposid{non-propagating-cache}@<remove_cv_t<@\exposid{InnerRng}@>> inner_; // \expos, present only when \tcode{!is_reference_v<\exposid{InnerRng}>}
  public:
    join_view() = default;
