...
namespace std {
  ...
  // \ref{format.err.report}, error reporting
  template<class charT, class... Args>
    struct @\exposid{basic-format-string}@;

  template<class... Args>
    using @\exposid{format-string}@ =                             // exposition only
      @\exposid{basic-format-string}@<char, type_identity_t<Args>...>;
  template<class... Args>
    using @\exposid{wformat-string}@ =                            // exposition only
...
\pnum
\constraints
\tcode{const T\&} models \tcode{convertible_to<basic_string_view<charT>>}.

\pnum
\effects
Direct-non-list-initializes \tcode{str} with \tcode{s}.

\pnum
\remarks
A call to this function is not a core constant expression\iref{expr.const}
unless there exist \tcode{args} of types \tcode{Args}
such that \tcode{str} is a format string for \tcode{args}.
\end{itemdescr}
...
Equivalent to:
\begin{codeblock}
return vformat_to(out, fmt.str, make_wformat_args(args...));
\end{codeblock}
\end{itemdescr}
...
Let
\begin{itemize}
\item \tcode{charT} be \tcode{decltype(fmt.str)::value_type},
\item \tcode{N} be
\tcode{formatted_size(fmt, args...)} for the functions without a \tcode{loc} parameter and
...
\begin{itemdescr}
\pnum
Let \tcode{charT} be \tcode{decltype(fmt.str)::value_type}.
